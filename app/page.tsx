"use client"
import { ChevronDown, FolderPlus, Plus, UserRoundSearch } from 'lucide-react';
...
import { getAllLeadLists } from './actions/leads';
...
export default function LeadsPage () {
   ...
   const [leadsCollections, setLeadsCollections] = useState<ExpandedLeadCollection[]>([]);
   const [folders, setFolders] = useState<string[]>(["All"]);

   async function loadAll () {
      const allLeadLists: any[] = await getAllLeadLists();
      const allFolders: any[] = await getFolders();
      setLeadsCollections(p => ([ ...allLeadLists ]));
   ...
   if (leadsCollections.length < 1) return <LoadingLeadsPage />

   return (
      <AppWrapper>
         <Spacing size={2} />
         <div className="text-l full pd-1 bold-600 dfb align-center gap-10">
            Hello. Find more leads today!
         </div>
            ...
            <Select
               ...
               onSelect={(folder) => {
                  ...
                  setLeadsCollections(prev => ([
                     ...prev.filter(leadList => leadList.folders.split(",").includes(selectedFolderName))
                  ]))
               }}
            />
            <input 
